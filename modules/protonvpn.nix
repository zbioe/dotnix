# from: emmanuelrosa/erosanix/modules/protonvpn.nix
{
  config,
  lib,
  ...
}:
# HOW TO USE IT (emmanuelrosa)
#
# To use it you first have to log on to your protonvpn account in a
# web browser and create a new Linux Wireguard configuration.
# From the generated configuration you just need the local peer's private key
# and the remote peer's IP address, to set the module's options.
# The private key needs to be stored in a file (ex. /root/protonvpn.key)
# so that it's not stored in the Nix store.
# Note that by default NixOS will not use the DNS server provided by
# ProronVPN. That requires extra configuration.
#
# SOURCE
# https://www.reddit.com/r/NixOS/comments/12ibbl9/protonvpn_nixos_setup/
